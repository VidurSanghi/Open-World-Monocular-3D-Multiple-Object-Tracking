...
def run_inference(image_dir, calib_dir, output_dir, split_file=None):
    ...
    image_paths = [p for p in image_paths if not (output_dir / (p.stem + ".npy")).exists()]
    ...
    for img_path in image_paths:
        try:
            ...
            depth = predictions[0]["depth_z"].squeeze().cpu().numpy()
            np.save(str(output_dir / (img_path.stem + ".npy")), depth)
            processed += 1
...
if __name__ == "__main__":
    ...
    run_inference(args.image_dir, args.calib_dir, args.output_dir, args.split_file)