...
# --- swap in REAL lidar ---
pts = np.fromfile(f'{DATA}/velodyne/{IDX}.bin', dtype=np.float32).reshape(-1,4)
...
print(f"Real LiDAR detections: {len(boxes)}")
...
ax.imshow(img)
colors = {1:'cyan', 2:'yellow', 3:'magenta'}
...
ax.set_title(f'Real LiDAR → PointPillars | {len(boxes)} detections | {IDX}')
ax.axis('off')
plt.tight_layout()
plt.savefig(f'/home/ubuntu/kitti-project/real_lidar_detections_{IDX}.png', dpi=150, bbox_inches='tight')
print("Saved real_lidar_detections.png")